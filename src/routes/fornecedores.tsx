    ...
    <div className="min-h-screen bg-[#f4f4f4]">
      ...
      <section className="max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-8 animate-in fade-in duration-500">
          {lista.map((f) => (
            <div key={f.id} className="bg-white rounded-3xl p-6 shadow-lg border border-zinc-100 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300">
              ...
              <span className="inline-block mt-2 bg-orange-100 text-orange-600 text-xs px-3 py-1 rounded-full font-semibold">{f.categoria}</span>
              <div className="mt-4 space-y-2 text-zinc-600">
                <p className="flex items-center gap-2 text-sm"><MapPin size={16} className="text-zinc-400" />{f.cidade}</p>
                <p className="flex items-center gap-2 text-sm"><Phone size={16} className="text-zinc-400" />{f.contato}</p>
              </div>
              <button className="mt-5 w-full flex items-center justify-center gap-2 bg-zinc-900 hover:bg-black text-white py-3 rounded-2xl font-semibold transition">
